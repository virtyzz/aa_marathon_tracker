...
export async function GET(request: Request) {
  try {
    ...
    const stats={xp:progresses.reduce((sum,p)=>sum+p.weekTask.xpSnapshot,0),todayXp:progresses.filter(p=>p.dayIndex===Math.max(0,Math.min(6,Math.floor((Date.now()-week.startsAt.getTime())/86400000)))).reduce((sum,p)=>sum+p.weekTask.xpSnapshot,0),completions:progresses.length};const characterXp=Object.fromEntries(characters.map(c=>[c.id,allProgresses.filter(p=>p.characterId===c.id).reduce((sum,p)=>sum+p.weekTask.xpSnapshot,0)]));
