...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      progresses, notes,
      stats: { xp: displayXp(totalXp(all)), selectedXp: characterWeeklyXp(progresses, week.characterXpResetAt, exclusions), todayXp: displayXp(totalXp(all.filter(p => p.dayIndex === today))), completions: all.length },
