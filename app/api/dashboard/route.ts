...
import { displayXp, displayMarathonXp, formatDayMonth, formatWeekRange, totalXp, weekDates } from "@/lib/marathon";
...
export async function GET(request: Request) {
  try {
    ...
    const [progresses, notes, weeks, all, marathonProgress] = await Promise.all([
      ...
      account ? prisma.characterTaskProgress.findMany({ where: { character: { gameAccountId: account.id } }, include: { weekTask: { select: { xpSnapshot: true } } } }) : [],
    ]);
    const exclusions = new Set((week.characterXpResetAt && characters.length
      ? await prisma.characterXpResetExclusion.findMany({ where: {
        characterId: { in: characters.map(c => c.id) },
        weekTaskId: { in: all.map(p => p.weekTaskId) },
      } }) : []).map(completionKey));
    ...
    return NextResponse.json({
      ...
      progresses, notes,
      stats: { xp: displayMarathonXp(totalXp(marathonProgress)), selectedXp: characterWeeklyXp(progresses, week.characterXpResetAt, exclusions), todayXp: displayXp(totalXp(all.filter(p => p.dayIndex === today))), completions: all.length },
      characterXp: Object.fromEntries(characters.map(c => [c.id, characterWeeklyXp(all.filter(p => p.characterId === c.id), week.characterXpResetAt, exclusions)])),
